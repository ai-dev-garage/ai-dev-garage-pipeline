...
2. Resolve `PROJECT_ROOT` from `project=<path>` in user input if present; otherwise use the current workspace root. Set `GARAGE_SEARCH_ROOTS` per install config.
3. Resolve path to `agents/deliver-task.md` by walking **GARAGE_SEARCH_ROOTS** in order. Assume that agent role and run its workflow, passing `$ARGUMENTS` as the task context.
