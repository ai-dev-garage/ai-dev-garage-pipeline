...
1. Copy **`jira.template.env`** in this folder to **`jira.env`** at **`~/.ai-dev-garage/`** or **`<project>/.ai-dev-garage/`**, then fill in values. The template may gain optional keys over time; merge new commented lines when you update the extension.
