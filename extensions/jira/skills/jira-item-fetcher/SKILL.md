...
1. Read **`$HOME/.ai-dev-garage/jira.env`** (`KEY=value` lines; skip `#` comments). Users can create it by copying **`jira.template.env`** from **`extensions/jira/`** in the pipeline repo.
2. If **`PROJECT_ROOT`** is set, read **`{PROJECT_ROOT}/.ai-dev-garage/jira.env`** the same way (overrides global file for keys it defines).
