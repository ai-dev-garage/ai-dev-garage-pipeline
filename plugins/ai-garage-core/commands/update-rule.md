...
1. If arguments are `help`, `-h`, or `--help`, output: "Updates an existing pipeline rule using the **rule-standard** skill. Provide the rule name or file path and a description of the desired changes. Optionally add `scope=plugin` (default) or `scope=project` to indicate where to look." Stop.
...
5. Apply the skill in **update** mode with: user input as change description, `TARGET_RULE_FILE`, `TARGET_ROOT`, `ASSET_SCOPE`. Output the proposed updated content. Do not write files until the user confirms.
