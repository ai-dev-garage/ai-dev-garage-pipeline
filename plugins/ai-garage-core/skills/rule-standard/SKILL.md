...
- **Where the asset lives or will be written** — supplied by the caller (`create-*` / `update-*` / `review-*` command or parent agent):
  ...
  - **`${CLAUDE_PLUGIN_ROOT}`** + rule name → `${CLAUDE_PLUGIN_ROOT}/rules/<name>.md`.
- Optional: **`ASSET_SCOPE`** (`plugin` | `project`).
...
3. **Secrets and credentials** — If the rule tells the model how to use tools, APIs, or local config, it must **not** encourage asking the user to paste passwords, tokens, or private keys in chat. It should require **environment variables** and/or **local gitignored** env files (e.g. `${CLAUDE_PLUGIN_ROOT}/skills/<skill-name>/<skill-name>.env`). This is **content** of the rule, not an exception to “no user interaction.” See [REFERENCE.md](references/REFERENCE.md) (“Secrets & credentials”).
4. **Plugin boundary** — Rules shipped from a plugin must not require the model to load or depend on **source assets outside** that plugin’s tree. **Cross-plugin** hard dependencies are unsupported for now. See [REFERENCE.md](references/REFERENCE.md) (“Plugin boundary”).
