...
- For **authoring** `create-*` / `update-*` / `review-*` commands: how the command resolves **`ASSET_SCOPE`** (`plugin` \| `project`) into **`TARGET_*`** paths and passes them into the loaded agent (see REFERENCE.md).
...
6. **Paths:** Outlines must **derive paths from explicit inputs** (`PROJECT_ROOT`, config path if any, **`${CLAUDE_PLUGIN_ROOT}`** or per-step resolved paths). **No hardcoded** home directory or repo `core/...` strings as the only way to load agents—commands map **scope → bundle root → file path** per REFERENCE.md. (Memory-specific paths remain **out of scope** until memory assets return.)
...
8. **Plugin boundary:** Commands authored within a plugin must not require loading agents/skills from **outside** that plugin’s tree as source. **Cross-plugin** hard dependencies are unsupported for now. See **[references/REFERENCE.md](references/REFERENCE.md)** (“Plugin boundary”).
...
4. **`create-*` / `update-*` / `review-*`:** The command must **compute and pass** the target location (e.g. **`TARGET_SKILL_DIR`**, **`TARGET_AGENT_FILE`**, **`${CLAUDE_PLUGIN_ROOT}`**) into the orchestrator prompt or agent inputs. **Global** vs **extension** is resolved here, not inside the standard skills.
