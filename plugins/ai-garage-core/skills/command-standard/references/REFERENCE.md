...
1. Parse **`ASSET_SCOPE`**: **`plugin`** (installed plugin) or **`project`** (project-local bundle).
2. Resolve to concrete roots using **plugin/config rules** (this REFERENCE does not hardcode directory names).
3. Pass into the loaded agent at least one of:
   - **`TARGET_SKILL_DIR`**, **`TARGET_AGENT_FILE`**, **`TARGET_COMMAND_FILE`**, etc., or
   - **`${CLAUDE_PLUGIN_ROOT}`** + asset kind + name, or
   - **`${CLAUDE_PLUGIN_ROOT}`** (ordered list) when the agent drafts resolution rules.

**Plugin** means “the installed plugin root” (whatever path the plugin system uses). **Project** means “project-local override bundle.” Exact paths are **never** the only truth inside a standard skill.

## Path resolution in command outlines (runtime)

Thin commands typically: resolve **`PROJECT_ROOT`** (e.g. `project=<path>` or workspace), resolve **`CONFIG_PATH`** if the install defines a project config file, resolve **`${CLAUDE_PLUGIN_ROOT}`** (project overrides take precedence over installed plugins), then load **`agents/<name>.md`** from the first root that contains it. **Spell this as an algorithm with variables**, not as a single hardcoded path.
...
1. Help branch  
2. Resolve target project (`PROJECT_ROOT`, `CONFIG_PATH` if used, **`${CLAUDE_PLUGIN_ROOT}`**)  
...
- Runtimes may **read** secrets from the **environment** or from **local gitignored** files. For Garage-installed skills, the canonical user file is **`${CLAUDE_PLUGIN_ROOT}/skills/<skill-name>/<skill-name>.env`** with precedence: env → project bundle → plugin bundle (unless stated otherwise).
- Do not outline steps that collect secret values in chat; point users to env vars and documented env files instead.

## Plugin boundary

- Commands within a plugin must not embed required paths or instructions that depend on **source assets outside** that plugin's tree.
- **Cross-plugin** hard dependencies are unsupported for now. Each plugin is self-contained.
- Install-time paths via caller-supplied variables (**`${CLAUDE_PLUGIN_ROOT}`**, **`TARGET_*`**) are fine.
...
2. Resolve `PROJECT_ROOT` (e.g. `project=<path>` or workspace). Set `CONFIG_PATH` and **`${CLAUDE_PLUGIN_ROOT}`** per plugin configuration.
3. Resolve path to `agents/my-orchestrator.md` by walking **`${CLAUDE_PLUGIN_ROOT}`** in order. Assume that agent role and run its workflow; pass **`ASSET_SCOPE`** and **`TARGET_*`** when creating or updating assets under a chosen bundle root.
````
...
- [ ] Paths are **derived from inputs** (`PROJECT_ROOT`, **`${CLAUDE_PLUGIN_ROOT}`**, **`TARGET_*`**) — not a single hardcoded install path
