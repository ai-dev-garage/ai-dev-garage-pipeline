...
1. If arguments are `help`, `-h`, or `--help`, output: "Updates an existing pipeline agent using the **agent-standard** skill. Provide the agent name or file path and a description of the desired changes. Optionally add `scope=global` (default), `scope=extension:<name>`, or `scope=project` to indicate which bundle to look in." Stop.
