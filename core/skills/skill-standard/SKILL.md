...
- **Where the asset lives or will be written** — never infer from this skill alone. The caller (**`create-*` / `update-*` / `review-*` command** or parent agent) must pass at least one of:
  ...
  - **`GARAGE_BUNDLE_ROOT`** — absolute path to a bundle root plus the skill name, so the package path is **`GARAGE_BUNDLE_ROOT/skills/<name>/`**.
- Optional context: **`ASSET_SCOPE`** (`global` \| `extension` \| `project`) so explanations match intent; the command maps scope to the paths above.
...
1. **Name:** Noun or noun phrase, lowercase, hyphenated. The skill folder name must match frontmatter `name` (Cursor requires this; Claude recommends it). Resolve the folder as **`TARGET_SKILL_DIR`** or **`GARAGE_BUNDLE_ROOT/skills/<name>/`** per caller input.
