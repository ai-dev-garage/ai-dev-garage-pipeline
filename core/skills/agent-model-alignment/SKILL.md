...
description: Read model effort mappings from project or global config and patch the model field in installed agent frontmatter files. Use after garage install/update or when model preferences change.
...
- After `garage install` or `garage update` resets agent files to extension defaults.
...
- `GARAGE_BUNDLE_ROOT` — path to the bundle root containing agents to align.
